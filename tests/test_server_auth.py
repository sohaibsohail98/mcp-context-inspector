...
def test_mcp_handshake_and_capability_probes_allowed_without_a_token(client):
    """A catalogue crawler whose MCP client can't attach an Authorization
    header (e.g. Glama's build-test inspector) must be able to complete
    the handshake and enumerate capabilities unauthenticated. None of
    these touch session data. See MultiTokenAuthMiddleware._MCP_DATA_METHODS."""
    for method in (
        "initialize",
        ...
        # then 400/202 on protocol details; that's past the auth gate,
        # which is the point.)
        assert resp.status_code != 401, f"{method} should not be blocked by the auth gate"


def test_mcp_data_calls_still_require_a_token(client):
    """The carve-out is a denylist: tools/call and resources/read (the
    methods that read/write session data) with no bearer token are
    still 401."""
    for method, params in (
        ("tools/call", {"name": "get_recent_sessions", "arguments": {}}),
...
def test_mcp_delete_without_token_still_401(client):
    """Session teardown (DELETE /mcp) is not part of the carve-out."""
    resp = client.delete("/mcp", headers={"mcp-session-id": "whatever"})
    assert resp.status_code == 401
