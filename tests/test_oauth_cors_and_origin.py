...
def test_oauth_register_preflight_succeeds_from_an_unlisted_origin(prod_client):
    """https://claude.ai is NOT in CHAT_UI_ORIGIN's allowlist — this must
...
def test_public_origin_env_var_overrides_request_base_url(prod_client, monkeypatch):
    ...
    monkeypatch.setenv("PUBLIC_ORIGIN", "https://mcp-inspector.example.com")

    resp = prod_client.get("/.well-known/oauth-authorization-server")
    body = resp.json()
    assert body["issuer"] == "https://mcp-inspector.example.com"
    assert body["token_endpoint"] == "https://mcp-inspector.example.com/oauth/token"

    resp = prod_client.get("/.well-known/oauth-protected-resource/mcp")
    assert resp.json()["resource"] == "https://mcp-inspector.example.com/mcp"
...
def test_401_www_authenticate_honors_public_origin(prod_client, monkeypatch):
    ...
    assert "https://mcp-inspector.example.com/.well-known/oauth-protected-resource/mcp" in resp.headers["www-authenticate"]
