...
connection, because MCP only ever sees calls made to our own tools,
never a client's own token usage. Gated by MultiTokenAuthMiddleware
exactly like /api/: "/otlp" is in its protected_prefixes, same
bearer-token-to-owner mapping, set via OTEL_EXPORTER_OTLP_HEADERS on the
client side. Body parsing and per-vendor mapping lives in
mcp_server/otlp/."""
...
# per-item processing/limiting happens. 25MB is well
...
async def otlp_debug(request: Request):
    ...
    _counts), so it resets on every redeploy/restart and a zero here
    after a fresh deploy doesn't by itself mean nothing has arrived since.

    Scoped to current_owner: each caller only ever sees their own
    counters and recent_skipped entries. This used to
